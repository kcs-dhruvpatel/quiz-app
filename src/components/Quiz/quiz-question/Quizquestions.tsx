...
const Quizquestions: React.FC = () => {
  ...
  let quizData: any = [];
  random_question.forEach(
    (element: { id: number; questionText: string; answerOptions: any }) => {
      quizData.push({
        id: element.id,
    ...
    <div className="App">
      {showScore ? (
        <div className="score-section">
          <h4>
            You scored {score} out of {quizData?.length}, You have scored{" "}
        ...
        <>
          <div className="question-section">
            ...
            {quizData && (
              <div className="question-text">
                QUE:{quizData[currentQuestion]?.questionText}
              </div>
            )}
          </div>
          <div className="answer-section">
            {quizData[currentQuestion]?.answerOptions.map(
              (
                answerOption: { isCorrect: boolean; answerText: string },
                id: number
              ) => (
                <button
                  onClick={() =>
